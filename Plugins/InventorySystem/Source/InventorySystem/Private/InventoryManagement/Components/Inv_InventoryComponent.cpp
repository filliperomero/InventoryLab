...
void UInv_InventoryComponent::Server_AddNewItem_Implementation(UInv_ItemComponent* ItemComponent, int32 StackCount)
{
	...
	// This is for cases where we play as a listen server or Standalone since we are doing the broadcast for clients in the FastArray::PostReplicatedAdd
...
void UInv_InventoryComponent::SpawnDroppedItem(UInv_InventoryItem* Item, int32 StackCount)
{
	...
	// RotatedForward is normalized so we need to multiply by some value to have a distance from the owning pawn
	...
	// & is important here since we would be making a copy of that
...
{
	if (bIsInventoryMenuOpen) CloseInventoryMenu();
...
void UInv_InventoryComponent::ConstructInventory()
{
	...
	checkf(InventoryMenuClass, TEXT("Inventory Menu Class is not set in: %s"), *GetName())
