...
void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
{
	...
	{
		const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(CurrentQueryResult.ValidItem.Get(), FragmentTags::Grid);
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		if (ShouldFillInStack(HoveredStackCount, RoomInClickedSlot))
		{
...
bool UInv_InventoryGrid::ShouldFillInStack(const int32 HoveredStackCount, const int32 RoomInClickedSlot) const
{
	return RoomInClickedSlot < HoveredStackCount;
}
...
void UInv_InventoryGrid::SetOwningCanvas(UCanvasPanel* InCanvas)
{
	OwningCanvasPanel = InCanvas;
}
...
void UInv_InventoryGrid::OnPopUpMenuDrop(int32 GridIndex)
{
	...
	PickUp(RightClickedItem, GridIndex);
	DropItem();
}
